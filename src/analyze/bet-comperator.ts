...
function calculateBetScore(bets: Bet[], hash_bet_score_map: Map<string, number>, outcome_distribution_map: Map<string, { home: number, draw: number, away: number }[]>, odds_in_percentage: { home: number, draw: number, away: number }[], bet?: Bet): number {
  let bets_sha_key = sha256(JSON.stringify(bets)).toString();//sha256(JSON.stringify(bets)).toString();
  let bets_and_bet_sha_key = sha256(JSON.stringify([...bets, bet])).toString(); // JSON.stringify([...bets, bet]);
  if (bet) {
  ...
  for (let game in outcome_distribution) {
    ...
    const outcomes_of_games = {
      home:  Math.round(((outcome_distribution[game].home | 0) / bets.length) * 10000) / 10000,
      draw:  Math.round(((outcome_distribution[game].draw | 0) / bets.length) * 10000) / 10000,
      away:  Math.round(((outcome_distribution[game].away | 0) / bets.length) * 10000) / 10000
    };
