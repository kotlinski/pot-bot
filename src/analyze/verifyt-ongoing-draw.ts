...
export const verifyBetsWithResults = async (
  game_type: string,
  ...
  try {
    ...
    for (let i = 0; i < correct_results.length; i++) {
      const correct_char = correct_results[i];
    ...
    const aggregated_values: AggregatedNoOfCorrects[] = [];
    correct_count.forEach((coupon) => {
